// pages/match-female/match-female.ts
// 女生发现页面
...
Page({
  ...
  loadCandidates() {
    ...
    // 注意：match-female 页面是用来查看女生的，所以当前用户应该是男生
    // 优先从存储中读取用户角色，如果没有则默认为男生（因为这个页面是给男生用的）
    const currentUserRole = wx.getStorageSync('userRole') || 'male_student';
    
    console.log('当前用户信息:', { 
      currentUserId, 
      currentUserRole, 
      userRoleFromStorage: wx.getStorageSync('userRole'),
      pageType: 'match-female (查看女生页面，当前用户应该是男生)' 
    });
    
    // 调用后端API获取匹配候选人（只返回男生）
    // 构建查询参数（GET请求需要将参数放在URL中）
    const params: string[] = [];
    if (currentUserId) params.push(`currentUserId=${encodeURIComponent(currentUserId)}`);
    params.push(`currentUserRole=${encodeURIComponent(currentUserRole || 'female')}`);
    params.push('page=1');
    params.push('limit=20');
    params.push('minScore=70');
    params.push('role=male_student'); // 目标性别：只看男生
